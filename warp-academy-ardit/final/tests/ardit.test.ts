...
describe('Testing the Atomic NFT Token', () => {
  ...
  beforeAll(async () => {
    ...
    ardit = warp.contract<ArditState>(contractId)
  ...
  it('should execute onnx model', async () => {

    const {result} = await ardit.viewState({ function: 'runOnnx'});
    console.log(`Onnex Result: ${result}`);
